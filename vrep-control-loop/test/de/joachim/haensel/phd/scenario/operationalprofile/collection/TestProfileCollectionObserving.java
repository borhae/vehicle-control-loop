...
public class TestProfileCollectionObserving
{
    ...
    public static Stream<Arguments> parameters()
    {
        return Arrays.asList(new Object[][] {
                // {"luebeck_small", 15.0, 120.0, 4.0, 4.3, 0.8, Arrays.asList(new
                // Position2D(5579.18,3023.38), new
                // Position2D(6375.32,3687.02)), "luebeck-roads.net.xml",
                // "blue"},
                // {"chandigarh_small", 15.0, 120.0, 4.0, 4.3, 0.8,
                // Arrays.asList(new Position2D(8564.44,9559.52), new
                // Position2D(7998.74,8151.80)), "chandigarh-roads.net.xml",
                // "blue"},
                // {"chandigarh_mini", 15.0, 120.0, 4.0, 4.3, 0.8, Arrays.asList(new
                // Position2D(6394.91,7882.57), new
                // Position2D(6497.73,7852.91)), "chandigarh-roads.net.xml",
                // "blue"},
                // {"chandigarh_medium", 15.0, 120.0, 4.0, 4.3, 0.8,
                // Arrays.asList(new Position2D(8564.44,9559.52), new
                // Position2D(7998.74,8151.80), new Position2D(7596.09,7264.80),
                // new Position2D(8158.54,3236.11), new
                // Position2D(11286.49,5458.54)), "chandigarh-roads.net.xml",
                // "blue"},
                // {"chandigarh_medium", 15.0, 120.0, 4.0, 4.3, 0.8,
                // Arrays.asList(new Position2D(7596.09,7264.80), new
                // Position2D(8256.48,3253.43), new Position2D(8135.55,3218.77),
                // new Position2D(8139.54,3115.05), new
                // Position2D(11286.49,5458.54)), "chandigarh-roads.net.xml",
                // "blue"},
                // {"luebeck_medium", 15.0, 120.0, 4.0, 4.3, 0.8, Arrays.asList(new
                // Position2D(4112.28,7084.47), new Position2D(6196.74,5289.38),
                // new Position2D(10161.11,3555.67), new
                // Position2D(3430.39,581.66), new Position2D(7252.29,1130.89)),
                // "luebeck-roads.net.xml", "blue"},
                // {"luebeck_mini_routing_challenge", 15.0, 120.0, 4.0, 4.3, 1.0,
                // Arrays.asList(new Position2D(7882.64,4664.21), new
                // Position2D(7797.34,4539.80), new Position2D(7894.70,4608.56),
                // new Position2D(8051.17,5536.44), new
                // Position2D(8039.89,5485.08)), "luebeck-roads.net.xml",
                // "blue"},
                // {"luebeck_supermini_routing_challenge", 15.0, 120.0, 4.0, 4.3,
                // 1.0, Arrays.asList(new Position2D(7894.70,4608.56), new
                // Position2D(8051.17,5536.44), new
                // Position2D(8039.89,5485.08)), "luebeck-roads.net.xml",
                // "blue"},
                { "luebeck_extramini_routing_challenge", 15.0, 120.0, 4.0, 4.3, 1.0, Arrays.asList(new Position2D(7882.64, 4664.21), new Position2D(7797.34, 4539.80)), "luebeck-roads.net.xml", "blue" },

                // {"luebeck_7_routing_challenge", 15.0, 120.0, 4.0, 4.0, 1.0,
                // Arrays.asList
                // (
            ...
            String mapFilenName, String color) throws VRepException
    {
        ...
        if (mapAndCenterMatrix != null)
        {
            ...
            final String localTestID = testID + String.format("%f_%f_%.2f_%.2f_%.2f_", lookahead, maxVelocity, maxLongitudinalAcceleration, maxLongitudinalDecceleration, maxLateralAcceleration);
            final Map<Long, List<TrajectoryElement>> configurationBuffers = new HashMap<>();
            ...
            writeConfiguration(configurationBuffers, localTestID);
            BiConsumer<? super Long, ? super ObservationTuple> writeObservation = (timeStamp, observation) ->
            {
                if (timeStamp != null)
                {
                    List<String> observationAsString = observation.toPyplotString();
                    String fileName = String.format("./res/operationalprofiletest/normalizedtrajectories/Observation%s%06d.pyplot", localTestID, timeStamp);
                    try
                    {
    ...
    private void writeConfiguration(final Map<Long, List<TrajectoryElement>> configurationBuffers, String testID)
    {
        Map<Long, List<TrajectoryElement>> normalizedTrajectories = TrajectoryNormalizer.normalize(configurationBuffers);
        ...
        Consumer<? super Entry<Long, List<TrajectoryElement>>> writeTrajectoryToFile = entry ->
        {
            try
            {
                if (entry.getValue() != null)
                {
                    ...
                    String fileName = String.format("./res/operationalprofiletest/normalizedtrajectories/Trajectory%s%06d.pyplot", testID, entry.getKey());
        ...
        normalizedTrajectories.entrySet().forEach(writeTrajectoryToFile);
    }
}
